...
def cnn(din , dout , stop_patience = 3, filter_num = 64, kernel_size_1 = (3,3), activation_1 = 'tanh'):
    checkpointer = callbacks.ModelCheckpoint(filepath='best_model_{}.h5'.format(filter_num), verbose=1, save_best_only=True)
    ...
    history = model.fit(batch_size=100, epochs=20, callbacks=[mycall, checkpointer], x=din.reshape(din.shape[0], 31, 31, 1), y = dout.reshape(dout.shape[0], 961), verbose=1)
    print(model.summary())
